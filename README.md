...
- **总结接口（小白x / SP·数据库 / 柚月记忆表 / 柏宝书）**：玩家面板"总结接口"页签可启用小白x剧情总结、SP·数据库 表格总结、yuzuki-Memory 记忆表总结或柏宝书剧情总结——分析时已总结楼层用其总结内容替代原文注入，总结边界之后的新楼层照常读取原文；周期位置、楼层计数、任务书签计算完全不受影响；未安装或无数据时自动回退纯原文模式。小白x 读聊天元数据里的结构化总结（事件/人物事实/剧情阶段）；SP·数据库 读其写入世界书的可读条目（总结表/重要人物/剧情大纲，按消息上的 TavernDB 表格快照判定边界）；柚月记忆表 读聊天元数据 `yuzukiMemory` 的记忆总结表（总结标题/核心角色/楼层数/总结内容/未解决问题，按各记录楼层区间并集的连续链判定边界，楼层有空洞时边界停在空洞前防误替换，剧情摘要时间线可一并注入、可在面板关闭）；柏宝书 经公开 API `globalThis.STBaiBaiBook` 读取当前状态快照（时间/地点/主角/变量树/物品/计划/场景/NPC/物品日志）与压缩剧情历史，以 `snapshot.point.floor` 为已总结边界，压缩历史可由面板开关关闭——均只读、零模块耦合、不修改插件数据；随配置模板导出/导入
